...
### [2026-05-23] — Per-channel sends to FX returns on BOTH mixers (input + output)
- Files (engine): `engine/include/ida/InputMixer.h`, `engine/src/InputMixer.cpp`, `engine/include/ida/OutputMixer.h`, `engine/src/OutputMixer.cpp`, `core/include/ida/MixerGraphState.h`.
- Files (UI): `app/MainComponent.cpp` (InputMixerPane + OutputMixerPane), per-strip sends row, `refreshInputDestinations` / `refreshOutputDestinations` (sends list build).
- What was deferred: when one or more FX-return buses exist on either mixer, EVERY channel strip on that mixer must expose a per-FX-return SEND control (level slider / send button) — the channel keeps its main-out untouched and additionally feeds a leveled copy of its post-fader signal into each FX return's input mixBuffer. Output Mixer has no FX returns today (the slice-3 surface only adds plain BusKind::Bus); design must extend to FX returns there too, or the request is asymmetric. Input mixer already has `MixerSend sends` on `InputChannelState` (see core/include/ida/MixerGraphState.h:113) but the persistence-engine round-trip wiring stops at FX-return enumeration; UI per-strip send row hasn't shipped. Output Mixer's `OutputChannelState` (slice 5a) has `sends` already used for the radio-style destination picker — semantics need to split into "main-out" (one of bus/master/hardware-output) vs "post-fader sends to FX returns" (any subset, with levels), so the radio picker's "all sends = 0 means HardwareOutput" inference breaks once FX-return sends become legitimate non-zero values.
- Why deferred: this is a substantial multi-touchpoint slice that needs its own design pass per `feedback_defer_big_design_to_own_session`. Specific open design questions for the operator before code lands:
    1. **Output Mixer FX returns** — does `OutputMixer::addBus` need a `BusKind::FxReturn` overload, and how do FX-return outputs route (back to master? to their own hardware pair? to another bus)?
    2. **UI surface** — per-strip sends row: where does it live in the narrow CompactFaderStrip? Below the INS button? In the detail panel beside pan/width? A dedicated "Sends" tab when N > some threshold?
    3. **Send semantics** — post-fader (channel mute silences sends) vs pre-fader (sends survive channel mute)? Operator-toggleable per channel?
    4. **Output Mixer main-out inference** — once sends-to-FX-returns are non-zero, the slice-5b "all sends = 0 means HardwareOutput" rule breaks. Need an explicit `channelMainOut(OutputChannelId)` engine accessor (mirror of `busMainOut(BusId)`) that tracks the picker choice independent of sends. Adds engine state OR persists current StripDest in MainComponent.
    5. **Cycle prevention** — channel → FX-return → ??? → channel is the same cycle class as bus → bus. The graph's `wouldMainOutCycle` is bus-bus; sends are bus-bus too. FX-return outputs need cycle filtering against the originating channel's send list (any FX return that ultimately routes back to itself via this channel is excluded).
- What's needed to finish: dedicated design slice. Read `docs/IDA_Whitepaper_V8.md` sends + FX-return sections, draft `docs/superpowers/specs/2026-05-XX-fx-return-sends-design.md` with the five questions above resolved, get operator approval on the decomposition, then implement engine first, UI second, persistence third (mirroring the 5a/5b/5c shape).
