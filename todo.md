...
- Files (anticipated): `app/MainComponent.cpp` (InputMixerPane gesture + layout), `engine/src/InputMixer.cpp` / `engine/include/sirius/InputMixer.h` (bus/tape-target registry), `docs/design/mixer-design.md` + `docs/Sirius Looper Whitepaper V7.md` §5.2/§6 (spec).
- What was deferred (operator request, this session): the operator wants the performer to **dynamically add buses (any number) and tape outputs** directly from the Input Mixer. Interaction: a **long-press on any BLANK area** of the Input Mixer (not on a strip) pops a **small two-option menu — "Add a bus" / "Add a tape"**. Each pick creates a new destination on the fly; buses are unbounded ("as many as they want"). This extends the existing gesture vocabulary (long-press already opens the per-strip Split/Collapse menu; this is the blank-area sibling) and is the natural creation-side of the queued **tape-output routing** slice (item 3 above) — strips route TO these buses/tapes.
