...
- Files (anticipated):
  ...
  - `engine/include/ida/OutputMixer.h` (no API change expected; binding is internal).
  - `app/MainComponent.cpp` — OutputMixerPane: add `ottoInsButtons_` scaffolding (mirror `busStripInsButtons_`: construct in `appendOttoStripImpl`, clear in `setOttoStrips`, erase in `removeOttoStripByIndex`, visibility toggles in the two full-screen blocks, `resized()` insRow layout, `ottoInsButtonScreenArea(int)` accessor, public `onOttoInsertChainClicked`); MainComponent: `openInsertChainPopupForOttoChannel(int ottoOutputIndex)` (mirror of `openInsertChainPopupForChannel` at ~6499 but resolve `ottoChannelByOutputIndex_` → OutputChannelId and read/write via `outputMixer_->audioStripForChannel(chId)->effectChain()/setEffectChain()`), and wire `outputMixerPane_->onOttoInsertChainClicked = [this](int idx){ openInsertChainPopupForOttoChannel(idx); }`.
  - Optionally wire the EXISTING-but-stubbed phrase/MON INS popups in the same slice (`onPhraseInsertChainClicked` at MainComponent.cpp ~5239 and `onMonInsertChainClicked` ~5461 are empty `[](int){}` stubs today) so all output channels gain functional inserts together.
- What was deferred: operator asked (2026-05-29, during S6 T8 verification) for an INS button on OTTO strips "with the same options as the other channels." Root-cause investigation (systematic-debugging) found the real gap is deeper than a missing button: **output-mixer CHANNELS never process their insert chain at all.** `ChannelStrip::process` → `dispatchInserts()` → `host_->pumpSlot(...)` is inert when `host_ == nullptr`, and `OutputMixer::setEffectChainHost` binds the FX host to **buses only**, never to channels (and neither `addChannel` nor `setChannelStrip` binds it). So phrase/MON INS buttons are dead stubs and OTTO has no button. Input channels + output buses DO have functional inserts; output channels are the gap. Per memory [[project_mixer_routing_destinations_and_plugins]] ("up to 8 plugins on EVERY node"), output channels SHOULD host inserts — this is an incomplete implementation, not a design choice.
- **Open design concern (resolve in the slice's brainstorm):** the FX host is **shared** between the input and output mixers (`MainComponent::effectChainHost_` bound to BOTH `inputMixer_->setEffectChainHost` AND `outputMixer_->setEffectChainHost`, lines ~4300-4301), and `IEffectChainHost` is keyed by raw node id (`chId.value()` / `busId.value()`). Input-channel ids, output-channel ids, and output-bus ids come from independent counters and numerically overlap, so binding output channels into the shared host risks node-key COLLISION (e.g. input channel 1 ↔ output channel 1 ↔ output bus 1 all keying the same host entry → FX cross-talk / clobber). Audit whether the key scheme already namespaces (input-channel vs output-bus inserts coexist today — determine HOW that works) BEFORE binding output channels. If keys collide, the slice must add a namespacing scheme (e.g. mixer-side + kind tag in the key) — that is the real engine design work and why this is its own slice, not a T8 patch.
- Why deferred: operator chose "own slice, do it right" (2026-05-29). It's genuine engine work (host binding + node-key uniqueness) affecting every output channel, deserving brainstorm→spec→plan→review like S6 — not a button bolted on during S6's T8 verification. A visual-only INS button (present-but-inert, matching phrase/MON) was explicitly rejected as the half-baked pattern.
- What's needed to finish: brainstorm (`superpowers:brainstorming`) the node-key-uniqueness resolution first (audit the shared-host key scheme; confirm/fix collision across input-channel / output-channel / output-bus); then spec the OutputMixer channel host-binding + the UI scaffolding (mirror bus INS) + the `openInsertChainPopupForOttoChannel` helper; plan + implement + review per S6's subagent-driven cadence; operator-verify the INS popup opens, the chain persists (ChannelStrip inserts already round-trip via `entry.inserts = ce.strip->effectChain()`), AND audibly processes. Fold phrase/MON popup-wiring in if low-cost.

### 2026-05-29 — Full output-mixer solo (phrase + MON + bus, cross-group)
- Files: `app/MainComponent.cpp` (`OutputMixerPane::stripSoloChanged` — currently only the OTTO branch is wired; a new `recomputeOutputStripMutes`), `app/MainComponent.h` (per-group `*Soloed_`/`*Muted_` state vectors).
- What was deferred: working solo for phrase/MON/bus output strips, and making OTTO solo **cross-group** (soloing any output strip silences all non-soloed output strips across every group, like the input mixer does). Today only OTTO-band-local solo ships: soloing an OTTO strip silences other OTTO strips but leaves phrases/buses audible. Phrase/MON/bus strips still *show* a solo button (`hasSoloButton()` is true for their types) that is inert — `stripSoloChanged` only dispatches the `ChannelType::Master` (OTTO) branch.
- Why deferred: operator scoped the 2026-05-29 session to OTTO-band-local solo only (answering the "why no solo on OTTO strips" question); full output-mixer solo waits until the input and output mixers are feature-complete.
- What's needed to finish: extend the recompute to all output groups (broaden `anySolo` across phrase+MON+bus+OTTO), add `onPhraseSolo`/`onMonSolo`/`onBusSolo` callbacks + parallel `*Soloed_`/`*Muted_` state, route those strips' mute through the recompute too (mirror of how OTTO mute now routes through `recomputeOttoOutputStripMutes`), and wire the existing-but-inert solo buttons. Mirror `recomputeInputStripMutes()` (`app/MainComponent.cpp`). The OTTO infra from this slice (`ottoStripMuted_`/`ottoStripSoloed_`/`recomputeOttoOutputStripMutes`) folds straight in.


### 2026-05-29 — S6 T1 Minor findings (deferred from code-quality review)
- Files: `engine/include/ida/OutputMixer.h` (~lines 120-134, 444), `engine/src/OutputMixer.cpp` (~lines 123, 502, 1006, 1136), `core/include/ida/MixerGraphState.h` (~line 189), `persistence/src/SessionFormat.cpp` (~lines 1066, 1091).
- What was deferred (three Minor findings from the T1 code-quality review; landed verdict was APPROVED for Important+Critical, Minor deferred per S3c precedent):
  1. **Dated `S6 (2026-05-29)` tags** repeated across six callsites. Project convention (`~/.claude/CLAUDE.md` "Docs describe how, not when") says session numbers / dated tags belong in `git log`, not source comments. Surrounding code has plenty of legacy `// slice E2` / `// slice 5a` tags so this isn't a new sin; T1 added six. Strip during a follow-up cleanup sweep alongside the legacy tags or as its own focused pass.
  2. **Doc-comment duplication** between `MixerGraphState.h`'s `OutputChannelState::ottoSource` field doc and `OutputMixer.h`'s `setOttoSource`/`channelOttoSource` accessor docs. Both spell out the -1/-2/0..31 sentinel contract. Should reduce the accessor doc to one line ("sets/reads the `OutputChannelState::ottoSource` marker — see that field for the value contract"). Single source of truth survives a future S7 sentinel change.
  3. **`setOttoSource` is `noexcept`-void; `setChannelMainOutToHardwareOutput` (claimed-mirror) returns `bool`.** The T1 doc comment says "mirror of `setChannelMainOutToHardwareOutput`" but neither the noexcept-ness nor the return type matches. Either drop the "mirror of" claim from the doc or make `setOttoSource` return `bool` for caller-observable unknown-id misses (MainComponent's rebind pass may actually want the bool).
- Why deferred: per S3c precedent + `feedback_no_deferral_get_it_done` — Minor findings don't block landings, and this whole set is a 30-minute pre-T8 polish pass that doesn't gate the operator-visible work. Doing them inline would have churned the T1 fixup commit further.
- What's needed to finish: one focused commit on top of S6 (after T8 ships) that addresses all three. No design questions, no spec change — pure code-comment + signature polish.

### 2026-05-27 — OTTO Stereo Mix output (PLAN WRITTEN; queued behind M-OTTO-4 slices 4c + 4d)
- Files (anticipated): `otto-bridge/include/ida/OttoHost.h` (new `getOttoStereoMixLeft/Right()` accessors + `kOttoStereoMixSentinelIndex = -2`), `otto-bridge/src/OttoHost.cpp` (internal stereo buffer + per-block sum of indices 28..31 inside `renderBlock`), `tests/OttoHostStereoMixTests.cpp` (new — `[otto-host-stereo-mix]` tag), `tests/CMakeLists.txt`, `app/MainComponent.{h,cpp}` (new `addOttoStereoMixStrip` / `removeOttoStereoMixStrip` / `hasOttoStereoMixStrip` + `ottoStereoMixChannelId_` member), `app/MainComponent.cpp` (OutputMixerPane picker entry + listener dispatch + appendOttoStereoMixStrip + remove gesture).
- What was deferred: Operator surfaced 2026-05-27 — "we should also offer a stereo out from OTTO option ... all players out into a single stereo pair in addition to each player's stereo out." A 33rd picker entry "OTTO Stereo Out (master mix)" that sums OTTO's four `PlayerOut1..4` sub-buses (output indices 28..31) into a single stereo strip. Per-output strips (slice 4b) remain unchanged and co-exist with the stereo strip; not mutually exclusive (operator accepts the double-count if they add both).
- **Design plan:** `docs/superpowers/specs/2026-05-27-otto-stereo-mix-output.md` (Option B chosen — IDA-side sum in `OttoHost::renderBlock`, no OTTO-side change required; two slices S1 + S2; recommended sequencing AFTER M-OTTO-4 slices 4c + 4d so the stereo mix lands routeable + audible from the start).
- **Open design questions** (resolve at implementation time, not before):
  1. **What does OTTO's `OutputRouter::Mode::Stereo` actually sum?** Per S1's recommendation it sums `PlayerOut1..4` (28..31). If OTTO's real master sum touches instrument channels (0..23) or FX returns (24..27) directly without going through player buses, the IDA-side sum must mirror that exact accumulation — summing only player buses would silently miss content. Audit `processBlockWithMasterBus` in OTTO before writing the sum routine.
  2. **Should the stereo-mix strip be visually distinct from per-output strips?** Plan defaults to same `ottoStrips_` band (uniform dispatch). Alternative: pin next to master with a divider. Decide at S2 time.
  3. **Coexistence with OTTO `Mode::Stereo`.** Does `processGlobalMixer` still populate per-output buffers when OTTO is internally in Stereo mode? If not, IDA's sum reads stale data. Verify in S1.
- Why deferred: net-new feature on top of slice 4b; per [[feedback_defer_big_design_to_own_session]] the operator named it as "write this up in a new plan and add it to the todo." Sequencing rationale: slice 4d (transport-start) makes ANY OTTO strip audible; slice 4c (routing + persistence) lets ANY OTTO strip route to a chosen hardware-output pair AND survive save/load. Landing the stereo mix BEFORE those would ship yet another silent / unrouteable strip — see [[feedback_sirius_done_right_and_complete]].
- What's needed to finish: M-OTTO-4 slices 4c + 4d landed first; then read the plan doc; audit OTTO's master-sum path per Q1 above; implement S1 + S2 per the plan; bump nothing in OTTO (no cross-project change required for Option B); ctest add 2-3 new `[otto-host-stereo-mix]` cases.

### 2026-05-26 — File-input MIDI source (NEEDS SPEC; sister of Audio file-input)
- Files (anticipated): `core/include/ida/FileInputDescriptor.h` (new InputKind::Midi variant or InputKind enum extension), `audio/include/ida/FileMidiInputSource.{h,cpp}` (new — parallel to FileInputSource but reads .mid via juce::MidiFile → drives a MIDI event ring), `audio/include/ida/FileInputRegistry.h` (polymorphic source storage OR a sibling MidiFileInputRegistry), `engine/include/ida/InputMixer.h` (MIDI channel binding — currently file-input setters assume SignalType::Audio), `app/MainComponent.cpp` (file-picker accepts .mid; new strip type for MIDI; MIDI player window variant), `docs/IDA_Whitepaper_V10.md` §6.6/§7.2 (extend the "file input is a playlist" passage to include MIDI as a first-class playlist entry type).
...
- **Open design questions:**
  ...
  3. MIDI clock vs. file timestamp: when a .mid file plays, do its event timestamps run against the file's own tempo map, or against OTTO's transport (per [[project_otto_is_the_transport_source]])? Probably the file's own tempo by default with a "lock to transport" option — but that "lock" sub-feature inherits the transport-sync entry's OTTO blocker below.
  4. Playlist semantics: do MIDI playlists work the same as audio playlists (concatenate, loop scope)? Most likely yes.
- Why deferred: substantial scope — a new signal-type source kind needs design before implementation. Per [[feedback_defer_big_design_to_own_session]]. **Additionally:** practical work on this slice is deferred until OTTO is imported (per [[project_otto_is_the_transport_source]]) so the transport-sync sub-feature can be designed once rather than twice. The non-transport parts (polymorphism, MIDI channel binding, playlist semantics) could ship independently, but the operator's stated workflow expects transport-synced MIDI from day one.
- What's needed to finish: OTTO importable into IDA first; then brainstorm (`superpowers:brainstorming`) against whitepaper §6.6/§7.2 + the just-shipped audio file-input architecture (`docs/superpowers/specs/2026-05-25-file-input-design.md` + `2026-05-26-file-input-audio-routing-design.md`) + OTTO's transport API + the existing MIDI signal path; pick polymorphism strategy; write spec; plan + implement.

### 2026-05-26 — File-input Video source (NEEDS SPEC; sister of Audio + MIDI file-input)
- Files (anticipated): `core/include/ida/FileInputDescriptor.h` (InputKind::Video), `audio/include/ida/FileVideoInputSource.{h,cpp}` (new — wraps juce::VideoComponent or a custom decoder; provides BOTH video frame surface AND an audio sub-track), `app/include/ida/FileInputPlayerWindow.h` (gains a video display surface — likely a new VideoPlayerWindow variant), `engine/include/ida/InputMixer.h` (video's audio sub-track binds the same way as a file-audio source; video frames bypass InputMixer entirely), `ui/` (a `VideoCanvas` component — new), `docs/IDA_Whitepaper_V10.md` (extend §6.6 + add §7.3 video subsection).
...
- **Open design questions:**
  ...
  3. Video sync: video frames must be A/V-synced. If audio drives the clock (typical), use the existing FileInputSource ring playhead to drive frame display. The "lock the playhead to global transport" sub-feature inherits the transport-sync entry's OTTO blocker; basic intra-source A/V sync does not.
...
- Why deferred: large scope — entirely new media-handling pipeline; needs design before implementation. Per [[feedback_defer_big_design_to_own_session]]. **Additionally:** practical work is deferred until OTTO is imported (per [[project_otto_is_the_transport_source]]) so transport-synced video can be designed once. Basic per-source A/V sync (frames follow the source's own audio sub-track) does not depend on OTTO; only the global-transport-lock variant does.
- What's needed to finish: OTTO importable into IDA first; then brainstorm against whitepaper §6.6 + cross-platform constraints ([[feedback_mac_first_linux_windows_last]] — macOS first; AVFoundation is the path of least resistance; Windows/Linux land later via JUCE's `VideoComponent` abstractions); pick display-surface model; write spec; plan + implement.

### 2026-05-26 — File-input transport sync (BLOCKED ON OTTO IMPORT)
- Files (anticipated): `core/include/ida/FileInputDescriptor.h` (new `TransportSyncMode` enum field), `audio/include/ida/FileInputSource.{h,cpp}` (consume transport state; gate play/stop based on sync mode), `audio/include/ida/FileInputRegistry.h` (transport-state subscription seam), `app/FileInputPlayerWindow.{h,cpp}` (UI for sync-mode selection — combobox or three-state toggle), `app/MainComponent.cpp` (wire OTTO's transport callback to the registry), `docs/IDA_Whitepaper_V10.md` §6.6 (document the sync mode).
- What was deferred: Operator surfaced 2026-05-26 — "playback needs to be able to sync to IDA's transport for start/stop/ignore." Currently every file input has independent transport; the player window's ▶/⏸/⏹ are local to the source.
- **HARD BLOCKER (2026-05-26 mid-slice):** the original entry assumed IDA's LMC published transport state. It does not. `engine/include/ida/Lmc.h` is pure time — `nowSeconds()`, `nowSecondsFromSamples()`, `advanceBySamples()` — no `isPlaying` / start / stop / TransportState class anywhere in `engine/`, `audio/`, `core/`, or `app/`. Per the operator (2026-05-26) and memory [[project_otto_is_the_transport_source]]: **OTTO is the transport source for IDA.** When bundled OTTO is imported (per [[project_otto_as_output_mixer_source]] + [[project_otto_is_a_submodule_now]]), it supplies play/stop to IDA, and that's what file inputs subscribe to. Building a stub IDA-side `EngineTransport` now would be the half-baked-feature trap from [[feedback_sirius_done_right_and_complete]].
- **Open design questions** (resolve at OTTO-import time, not before):
  ...
  5. Persistence: TransportSyncMode persists per file input (FileInputDescriptor field — session round-trip is already wired so this is a one-line extension).
...
- Why deferred: hard-blocked on OTTO import. Cannot proceed until OTTO is consumable as IDA's transport source.
- What's needed to finish: (1) OTTO importable into IDA — see [[project_otto_as_output_mixer_source]] / [[project_otto_is_a_submodule_now]] for the OTTO-integration prerequisites. (2) Identify the OTTO API that publishes transport state (likely a callback / listener on the bundled OTTO instance). (3) Then resume from "Open design questions" above, write spec, plan, implement. Until (1) lands, no productive work on this entry is possible.
...
### [2026-05-23] — Per-channel sends to FX returns on BOTH mixers — design APPROVED, ready to implement
- Spec: `docs/superpowers/specs/2026-05-23-mixer-symmetry-fx-returns-sends-design.md`. All five open questions resolved (D1–D5).
...
### [2026-05-28] — RESOLVED: IDA's OTTO audio pump — landed as S3c at IDA `f4111bba` + OTTO `ee390098`
...
### [2026-05-30] — T0b playback wiring v1 limits (MainComponent glue)
- Files: `app/MainComponent.cpp` (`refreshOutputMixerPhraseChannels`, `resolveLoopTapeInfo`), `app/MainComponent.h`
- What was deferred (not blocking, document only):
  1. **tapeIn-relative wrap.** The prefetcher wraps the read cursor from sample 0 (`s %= loopLengthSamples`), which is correct when a loop's tape slice begins at 0 (the common "recorded loop = whole tape" case). A loop whose `tapeIn != 0` wraps to 0 instead of the slice-relative tapeIn offset — the prefetcher needs a slice-start offset parameter to handle non-zero tapeIn correctly.
  2. **Seek-on-drain latency (inherited from Task 5 above).** A transport jump is honored only after the ~1 s ring drains — same as the standalone Task 5 item.
  3. **Silent pills whose tape file does not yet exist.** A pill whose tape has never been recorded stays silent (prefetcher open fails, channel stays in the OutputMixer but zero-fills the scratch). There is no automatic retry when the tape appears; only a subsequent `refreshOutputMixerPhraseChannels` call (triggered by the next session edit) will re-attempt the open.
