...
### 2026-05-22 — InputMixer ctor seeds RVB+DLY FX returns; violates minimal-defaults rule
- Files: `engine/src/InputMixer.cpp:41-42` (the two `addFxReturn(...)` calls); `engine/src/InputMixer.cpp:259-314` (the Phase-5 persistence `importGraphState` path that special-cases the ctor-seeded RVB(busId 1)/DLY(busId 2) and the comment "ctor already set nextBusId_ == 3"); `engine/include/sirius/InputMixer.h:128` doc comment about "the ctor's RVB/DLY"; any Catch2 test that assumes RVB/DLY exist at `InputMixer{}` construction.
- What was deferred: rule (operator-stated 2026-05-22, memory `project_minimal_default_mixers`): both mixers start MINIMAL — channels matching physical I/O ONLY, no buses, no FX returns, no RVB/DLY. Presets (a later feature) will recall common configs in one gesture. The InputMixer ctor currently seeds `addFxReturn("RVB")` + `addFxReturn("DLY")` directly, contradicting the rule. OutputMixer already starts empty (no change needed).
- Why deferred: this is its own slice. The removal cascades into: (a) tests that assume RVB(busId 1)/DLY(busId 2) exist at startup, (b) Phase-5 persistence import that special-cases skip-create for the seeded pair and starts `nextBusId_` at 3, (c) the existing P6 blank-area "Add FX return" gesture must continue to mint these on demand (already works — confirmed in P6 T2). Touches engine + tests + persistence path together; doing it inside the docs-update commit would be too broad.
- What's needed to finish: a dedicated slice (TDD on engine): delete the two ctor `addFxReturn` calls; reset `nextBusId_` invariant to 1 (fresh mixer); update `importGraphState` to NOT skip-create RVB/DLY busIds (treat ALL persisted buses uniformly) — minus a careful migration story for sessions saved with the old ctor-seeded ids 1+2; sweep tests that referenced the seeded returns to add them via `addFxReturn` instead; update the `InputMixer.h:128` doc comment. Docs already reflect the rule (whitepaper §6.1, routing-graph spec, mixer-design.md, memory). Recommend scheduling this AHEAD of P7 so P7's Sends-tab UI doesn't bake an assumption that returns always exist.
