...
### [2026-05-28] — IDA's OTTO audio-pump skips processBlock — AudioMessage queue never drains, MIDI dispatch + sample render never run

- Files: `otto-bridge/src/OttoHost.cpp` (renderBlock at line 234-240), `external/OTTO/src/otto-plugin/PluginProcessor.cpp` (processBlock at line 665, processAudioMessages at line 1923).
- What was deferred: the whole IDA-side OTTO audio pump. Today `OttoHost::renderBlock` calls only `processor->getPlayerManager().processGlobalMixer(numSamples)` — the channel/bus sum + meter stage. The rest of OTTO's audio thread lives in `OTTOProcessor::processBlock`: `processAudioMessages()` (drains the SPSC queue where Play/Stop/TempoChange land), `updateTransportState()` (broadcasts TransportEvent via EventBus to IDA's bar listener), `pinEnginesForBuffer()`, conductor advance, MIDI dispatch through every Player, sample render via sfizz. **None of that runs.** Therefore `host_.play()` / `host_.stop()` / `host_.setTempo()` post AudioMessages into a queue that never drains; even if the queue drained, the conductor never advances and sfizz never renders. Net effect: pressing Play in the bar does nothing audible; the bar's transport-state echo never fires (the listener only sees broadcasts from `updateTransportState`).
- Why deferred: this is architectural, not bounded — it asks "how does IDA route OTTO's full audio output through its own mixer model." The current `processGlobalMixer`-only call was a deliberate S2-era choice (design comment at `OttoHost.cpp:84-95` and `:230-238`) presumably to avoid OTTO's own master rendering competing with IDA's Output Mixer. Reversing that choice requires deciding: (a) does IDA call `processor->processBlock` and intercept OTTO's output buffer for its own routing? (b) does OTTO grow a non-rendering `drainAudioThreadState(numSamples)` API that splits processBlock's housekeeping from its rendering? (c) hybrid? Per `feedback_defer_big_design_to_own_session`.
- T5/T6/T7/T8/T9 in-tree headless tests pass because they verify the WIRE (host_.play() returns without crashing, listener subscribes, etc.) — they never verify that Play actually starts playback. T10 + T14 are the first time the wire was end-to-end-tested; both fail at the same point.
- What's needed to finish: a fresh brainstorm + design session — start by reading `external/OTTO/src/otto-plugin/PluginProcessor.cpp::processBlock` end-to-end (line 665-~900), enumerating which steps IDA needs (drain, transport update, MIDI dispatch, sample render) vs which IDA must replace (master rendering, output buffer routing). Outcome is a spec + plan that lands in its own slice. S3b shipped the AssetsRoot wire (real value — kit picker now populated); this gap is the next blocker for operator-audible OTTO playback inside IDA.
