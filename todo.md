...
### 2026-05-29 — S6 T7 must seed ottoStripMuted_ from restored engine mute on load
- Files: `app/MainComponent.cpp` (`chooseFileAndLoad` / wherever S6 T7 calls `rebindOttoChannelsAfterImport` and recreates OTTO strips via `setOttoStrips`), `app/MainComponent.h` (`ottoStripMuted_`).
- What was deferred: after the OTTO-solo slice (2026-05-29), `recomputeOttoOutputStripMutes()` is the **single writer** of each OTTO channel's engine mute, driven by `ottoStripMuted_` (which initializes all-false). `OutputChannelState.muted` IS persisted (`persistence/src/SessionFormat.cpp:960/982`). When S6 T7 lands the live load-rebind path, a saved-muted OTTO strip will restore its engine mute, but `ottoStripMuted_` will still be false — so the FIRST solo/mute interaction after load would clobber the restored mute back to unmuted.
- Why deferred: not a live bug today — verified no live path currently restores OTTO strips on load (`setOttoStrips` has no caller; `rebindOttoChannelsAfterImport` is not called from MainComponent; S6 T7 is queued, not landed). The fix must live inside T7's load path, which does not exist yet.
- What's needed to finish: in the T7 load/rebind path, after the OTTO channels are rebound, seed `ottoStripMuted_[ottoOutputIndex]` from each restored channel's engine mute (or from the imported `OutputChannelState.muted`), then call `recomputeOttoOutputStripMutes()` to reconcile engine + visual. Solo state is transient (not persisted) and correctly stays all-false on load.
