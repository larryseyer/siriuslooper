...
| **Decisions** | Input source format (mono / stereo, split/collapse) · Source enable · Source-level defaults · File transport (start, rate, loop region) **— transport applies to the playlist entry currently active; advance to the next entry is governed by playlist scope (off / track / list)** | Channel processing chain · Tape mode (commit-to-tape / non-destructive / no-tape) — i.e. **wet tap / dry tap / no tape** (§6.3.2) · **MON** on/off (§6.3.1) · Destinations (tapes, buses) |
...
- **Both mixers carry this, and the performer builds it live.** The same bus/FX-return/send structure exists on the input mixer too — there its outputs terminate at tape (monitoring reaches the physical outputs via the direct layer to an output-mixer channel, never by the input mixer writing outputs directly) rather than only at the master — and buses and FX returns **appear because the performer creates them on demand**, the same principle by which tapes appear because channels demand them. Each mixer carries any returns the performer adds; **nothing is seeded by default** (see §6.1.1). Input-side buses and FX returns reach the direct layer via the same **MON** toggle the per-channel surface carries (§7.2): on a bus or FX return, MON taps the post-processing signal and presents it as a peer output-mixer channel, just as per-channel MON does. Without that toggle, a summing bus on the input side would have no path to be heard. The interaction model and engine design live in the mixer design docs.
...
The performer's musical content is portable via stems and MIDI files; the architectural content is preserved in the session file itself. **Two complementary exchange formats: stems for everyone, the session file for any IDA-compatible tool.** DAW project exchange is treated as out of scope in the same spirit as cross-application audio routing (§1.6) — the operating system and the receiving application can handle the translation if and when they care to.
...
## 7.2 The MON toggle — one mode, per input-side node, post-strip

The direct layer offers exactly one monitoring mode per input-side node: **MON**. Every audio-producing node on the input mixer carries it — input channels, aux buses, and FX returns. The mechanism is identical in all three cases; only the tap point's source differs.

**Tap point.** When MON is on for an input-side node, the signal is tapped **post-node** — after the node's processing chain (gain staging, EQ, dynamics, inserts). For a channel, that's post-strip; for a bus or FX return, that's after the bus's summed-and-processed output. The performer hears the node's signal *with* the processing the operator has applied. This is the default and only direct-layer mode in IDA.

**Destination.** The post-node signal arrives at an **auto-created channel in the output mixer** (§5.2, §6.6). That output-mixer channel is a peer of the per-phrase channels — it appears when MON is on, vanishes when MON is off, carries gain, pan, inserts, and routing like every other output-mixer channel, and is mixed alongside phrase playback through master. The performer hears the input-side signal mixed with their loops, through the same signal path, with no separate monitor bus or control-room section.

**Why buses need MON too.** An input bus that sums signals (a drum subgroup, a vocal subgroup) or an FX return that hosts a reverb has no architectural way to be heard otherwise: the input mixer never writes physical outputs directly (§6.6), and the bus's only main-out targets are tape or another bus. Without MON on the bus, the summed signal lands on tape but is silent in the room. The per-bus MON toggle is the bridge — the same direct-layer mechanism that carries per-channel monitoring also carries per-bus monitoring, with a peer output-mixer channel labelled by the source bus's name.
...
**Independence from tape.** MON is decoupled from tape capture. The same input channel may capture to tape with MON off (the *amp-in-the-room* case) or carry MON without committing (rehearsal). See §6.3.1 for the orthogonality table and §6.3.2 for what tape captures (wet tap or dry tap) when Tape is on. The same orthogonality holds for buses: a drum subgroup can route to tape AND be heard via MON simultaneously, with neither toggle dragging the other.

**Independence from the strip's mute.** The strip's mute is the operator-facing kill-switch and silences MON regardless of where in the chain the tap sits. Muting an input is always heard; MON cannot defeat a mute. Same rule on buses — muting the bus silences both its tape route and its MON tap.
...
**File I/O** — One of the four signal modalities handled as first-class by both mixers. Audio, MIDI, and video files entering or leaving the session. Architecturally identical to live signals; a file input is a tape that already exists, and file export is just playback aimed at a file destination. A file input is an ordered playlist of one or more files; the playlist may be edited live during playback.
...
**MON** — The single per-node direct-layer toggle on an input-side strip (channel, aux bus, or FX return). When on, the node's post-processing signal is tapped into the direct layer and arrives at an auto-created channel in the output mixer, where it is mixed alongside phrase playback. Independent of tape (see §6.3.1 for the MON × Tape truth table). Adds the node's processing chain's few milliseconds of latency; IDA does not provide a sub-millisecond raw-direct mode (§7.2). Without MON on a summing input bus, the bus is silent in the room (it can still write to tape).
