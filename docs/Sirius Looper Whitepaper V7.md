...
**Audio is stereo, always.** Every audio channel, bus, send, output, and master in either mixer is a **stereo pair** — Sirius, like its sister app OTTO, has no mono audio path anywhere. A mono source (a single mic, one side of an interface) is presented and processed as stereo (dual-mono) from the channel boundary inward; the operator never sees or routes a mono audio channel or bus. Pan operates within the stereo field; metering is per-side (dual meters). This is a hard invariant, not a default — engine routing, mixer UI, and metering all assume stereo pairs. (It applies to the audio modality; MIDI and video channels carry their own native payloads.)
