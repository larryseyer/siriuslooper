...
- **Both mixers carry this, and the performer builds it live.** The same bus/FX-return/send structure exists on the input mixer too — there its outputs terminate at tape rather than at the master — and buses and FX returns **appear because the performer creates them on demand**, the same principle by which tapes appear because channels demand them. The interaction model and engine design live in the mixer design docs.
