...
**2. `PluginStateRegion` lives in `core/`, alongside `PluginGuiState`.** Both are POD shared-memory layouts that the child and the engine each `view` over. Their location precedent is `core/include/sirius/PluginGuiState.h` (no implementation .cpp — the type is header-only). M8 S2's `PluginStateRegion` follows the same shape: header-only layout in `core/include/sirius/PluginStateRegion.h`, no `.cpp`.
...
**6. The third-party integration test is opt-in via CMake variable, not just env var.** `SIRIUS_THIRDPARTY_CLAP_PATH` is checked at CMake configure time; when set, the test target is built and the ctest case registered. When unset, the test is silently absent. This matches the existing `SIRIUS_SYNTHETIC_CLAP_PATH` convention (`tests/fixtures/CMakeLists.txt` and the `openSyntheticTestPlugin` debug button).
...
| Path | Responsibility |
|---|---|
| `host/include/sirius/ClapBundleLoader.h` | RAII loader public surface — load, descriptors, createPlugin |
| `host/src/ClapBundleLoader.cpp` | dlopen + entry init + factory + descriptor walk |
| `host/include/sirius/ClapScanner.h` | CLAP scanner public surface — defaultSearchPaths, scan |
| `host/src/ClapScanner.cpp` | Path walker + per-bundle loader invocation |
| `core/include/sirius/PluginStateRegion.h` | `PluginStateState` shm POD layout + `makeStateRegionName` |
...
| `tests/ThirdPartyClapIntegrationTests.cpp` | 2 conditional cases on `SIRIUS_THIRDPARTY_CLAP_PATH` |
...
| `host/include/sirius/OutOfProcessPluginInstance.h` | + `requestStateSave` / `requestStateLoad` / `stateRequestSeq` / `stateResponseSeq` |
| `host/src/OutOfProcessPluginInstance.cpp` | + state shm region construction; method bodies |
| `host/include/sirius/OutOfProcessEffectChainHost.h` | + `descriptorForSlot` / `stateBlobForSlot` |
| `host/src/OutOfProcessEffectChainHost.cpp` | method bodies + timeout-notification post |
| `host/include/sirius/PluginScanner.h` | + `setNotificationSink` |
| `host/src/PluginScanner.cpp` | Remove FabFilter filter; add CLAP scan pass; add progress notifications |
| `engine/include/sirius/SessionSnapshot.h` | + `SlotLocation`; signatures gain host + lookup callback |
...
**Files:**
- Create: `host/include/sirius/ClapBundleLoader.h`
...
- Modify: `tests/CMakeLists.txt` (add `ClapBundleLoaderTests.cpp` to `SiriusTests` sources)
           ...
           "[clap-bundle-loader]")
{
    ...
    auto loader = ClapBundleLoader::load (
        SIRIUS_HOST_BINARY_PATH "/Contents/MacOS/sirius_plugin_host", err);
    CHECK_FALSE (loader.valid());
    CHECK_FALSE (err.empty());
}
...
Append `ClapBundleLoaderTests.cpp` to the `SiriusTests` source list. Find the existing list (search for `ArchivalModeTests.cpp` to locate the right block) and add the new file alphabetically near it.

`SIRIUS_HOST_BINARY_PATH` must be passed through to the test — it already is for the OutOfProcess tests; verify by greping the existing CMakeLists for `SIRIUS_HOST_BINARY_PATH` and following the same pattern.
...
1. Add `#include "sirius/ClapBundleLoader.h"` near the other sirius includes (after `sirius/SharedMemorySpscQueue.h`).
...
```bash
cmake --build build --target sirius_plugin_host SiriusTests
...
```bash
git add host/include/sirius/ClapBundleLoader.h host/src/ClapBundleLoader.cpp \
...
**Files:**
- Create: `host/include/sirius/ClapScanner.h`
...
Append `ClapScannerTests.cpp` to the `SiriusTests` source list alphabetically near `ClapBundleLoaderTests.cpp`.
...
```bash
git add host/include/sirius/ClapScanner.h host/src/ClapScanner.cpp \
...
**Files:**
- Modify: `host/include/sirius/PluginScanner.h` (add `setNotificationSink`)
...
- [ ] **Step 3.2: Modify `host/include/sirius/PluginScanner.h`**
...
```bash
git add host/include/sirius/PluginScanner.h host/src/PluginScanner.cpp
git commit -m "$(cat <<'EOF'
...
**Files:**
- Create: `core/include/sirius/PluginStateRegion.h`
...
Append `PluginStateRegionTests.cpp` to the `SiriusTests` source list.
...
```bash
git add core/include/sirius/PluginStateRegion.h tests/PluginStateRegionTests.cpp tests/CMakeLists.txt
git commit -m "$(cat <<'EOF'
...
`runClapMode` now takes `sirius::PluginStateState* stateState` as a parameter. Add:
...
**Files:**
- Modify: `host/include/sirius/OutOfProcessPluginInstance.h` (add the two methods + region members)
...
Append `PluginStateExtensionTests.cpp` to the `SiriusTests` source list.
...
- [ ] **Step 7.4: Modify `host/include/sirius/OutOfProcessPluginInstance.h`**
...
```bash
git add host/include/sirius/OutOfProcessPluginInstance.h \
...
**Files:**
- Modify: `host/include/sirius/OutOfProcessEffectChainHost.h` (add the two accessors)
...
- [ ] **Step 8.4: Modify `host/include/sirius/OutOfProcessEffectChainHost.h`**
...
```bash
git add host/include/sirius/OutOfProcessEffectChainHost.h \
...
- `StatefulSynthFixture.plist.in`: identical structure; identifier `com.sirius.synthetic.statefulsynth`, bundle name `StatefulSynthFixture`.
...
Find where `SIRIUS_SYNTHETIC_CLAP_PATH` is set as a generator-expression define on `SiriusTests` and add a sibling:

```cmake
target_compile_definitions(SiriusTests PRIVATE
    SIRIUS_STATEFUL_SYNTH_CLAP_PATH="$<TARGET_BUNDLE_DIR:StatefulSynthFixture>")
add_dependencies(SiriusTests StatefulSynthFixture)
```
...
```bash
cmake --build build --target StatefulSynthFixture SiriusTests
...
**Files:**
- Modify: `engine/include/sirius/SessionSnapshot.h` (add `SlotLocation`; new signatures)
...
Read `engine/include/sirius/SessionSnapshot.h` first to see the M8 S1 shape. Then replace the function declarations:
           ...
           "[session-snapshot-live]")
{
    ...
    auto root = sirius::Constituent::makeRoot();
...
```bash
git add engine/include/sirius/SessionSnapshot.h \
...
```cpp
const auto lookup = [this] (const sirius::Constituent&,
                            std::size_t entryIndex)
    -> std::optional<sirius::SlotLocation>
{
...
codesign --verify --deep --strict build/app/SiriusLooper.app
...
## Task 12: `SIRIUS_THIRDPARTY_CLAP_PATH` conditional integration test

**Files:**
- Create: `tests/ThirdPartyClapIntegrationTests.cpp`
- Modify: `tests/CMakeLists.txt` (conditional registration on `SIRIUS_THIRDPARTY_CLAP_PATH`)
...
Locate where `SiriusTests` is defined. Append:

```cmake
if(DEFINED SIRIUS_THIRDPARTY_CLAP_PATH)
    target_sources(SiriusTests PRIVATE
        ThirdPartyClapIntegrationTests.cpp)
    target_compile_definitions(SiriusTests PRIVATE
        SIRIUS_THIRDPARTY_CLAP_PATH="${SIRIUS_THIRDPARTY_CLAP_PATH}")
endif()
```
...
cmake -B build -S . -G Ninja \
    -DSIRIUS_THIRDPARTY_CLAP_PATH=/Library/Audio/Plug-Ins/CLAP/FabFilter\ Pro-Q\ 3.clap
cmake --build build --target SiriusTests
...
git commit -m "$(cat <<'EOF'
feat: M8 S2 — SIRIUS_THIRDPARTY_CLAP_PATH integration test (FabFilter/Surge/etc.)
...
- Spec §Scope item "Conditional `SIRIUS_THIRDPARTY_CLAP_PATH` integration test" → Task 12.
