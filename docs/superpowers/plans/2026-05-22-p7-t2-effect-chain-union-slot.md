...
**Create:**
- `core/include/sirius/InternalFxId.h` — `enum class InternalFxId : uint8_t` + free `toString` / `fromString` helpers.
- `tests/InternalFxIdTests.cpp` — pin the enum values + the to/from-string round-trip.

**Modify:**
- `core/include/sirius/EffectChain.h` — add `EffectChainSlotKind` enum, add `kind` + `internalId` fields to `EffectChainEntry`, add `makePlugin` / `makeInternal` factory static members, update `operator==`.
...
**Out of scope (T3 and later):** runtime adapter classes, OTTO Player FX wrappers, RT-safe param swap, any `engine/src/fx/` files, any UI surface. Nothing in `app/`, `ui/`, `engine/include/sirius/InputMixer.h` interface changes, `MixerGraphState.h` (its `inserts` field is already an `EffectChain` and inherits the new shape for free).
...
**Files:**
- Create: `core/include/sirius/InternalFxId.h`
...
Run:
```bash
cmake --build build --target SiriusTests && ctest --test-dir build -R InternalFxId --output-on-failure
```
...
```bash
git add core/include/sirius/InternalFxId.h tests/InternalFxIdTests.cpp tests/CMakeLists.txt
...
**Files:**
- Modify: `core/include/sirius/EffectChain.h` (add `EffectChainSlotKind` enum, two new fields + two factory statics, update `operator==`)
...
Run:
```bash
cmake --build build --target SiriusTests 2>&1 | head -40
```
...
Edit `core/include/sirius/EffectChain.h`. Add the includes + new enum + factory declarations + the two new fields. The full updated struct:
...
Run:
```bash
cmake --build build --target SiriusTests && ctest --test-dir build -R "effect-chain|effectchain|union-slot" --output-on-failure
```
...
```bash
git add core/include/sirius/EffectChain.h core/src/EffectChain.cpp tests/EffectChainTests.cpp persistence/src/SessionFormat.cpp
...
Run:
```bash
cmake --build build --target SiriusTests && ctest --test-dir build -R "union-slot" --output-on-failure
```
...
Run:
```bash
cmake --build build --target SiriusTests && ctest --test-dir build -R "union-slot|effect-chain|effectchain|sessionformat" --output-on-failure
```
           ...
           "[sessionformat][mixer][union-slot]")
{
    ...
    sirius::engine::InputMixer source;
    ...
    const auto json  = sirius::persistence::serializeMixerGraphState (state);
    const auto round = sirius::persistence::deserializeInputMixerGraphState (json);
           ...
           "[sessionformat][mixer][union-slot]")
{
    ...
    sirius::engine::OutputMixer source;
    ...
    const auto state = source.exportGraphState();
    const auto json  = sirius::persistence::serializeMixerGraphState (state);
    const auto round = sirius::persistence::deserializeOutputMixerGraphState (json);
...
Run:
```bash
cmake --build build --target SiriusTests && ctest --test-dir build -R "mixer.*union-slot" --output-on-failure
```
...
- [ ] `grep -n 'EffectChainSlotKind::Plugin' core/include/sirius/EffectChain.h` shows the new enum is referenced.
