...
First add `#include "ida/NotificationBus.h"` to the test file's include block (it is not included by Task 1).
...
After the existing includes (top of the file, after the `#include "ida/IEffectChainHost.h"` line), add a forward declaration inside `namespace sirius` (before `class Bus`):
...
`ChannelId` is already available via `#include "ida/Channel.h"` (already included by Bus.h).
...
If `Bus.cpp` does not already include `sirius/Rational.h` transitively, add `#include "ida/Rational.h"` at the top. (It is available via `Channel.h`/`TapeWriter` paths in engine; confirm it compiles.)
