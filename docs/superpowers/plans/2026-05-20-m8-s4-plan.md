...
**Tech Stack:** C++20, JUCE (`juce_core`), CMake/Ninja, Catch2 (`SiriusTests`). RT-safety per `docs/RT_SAFETY_CONTRACT.md`.
...
- `engine/include/sirius/IWetCaptureSink.h` *(new)* — pure audio-thread interface, JUCE-free. One responsibility: the contract `Bus` calls to hand off wet audio.
- `engine/include/sirius/WetCaptureWriter.h` *(new)* — the writer's public surface + `WetWriteMessage` POD.
...
- `engine/include/sirius/Bus.h`, `engine/src/Bus.cpp` *(modify)* — the set-once sink pointer + the post-chain tap.
...
Reused as-is: `LockFreeSpscQueue<T>` (`engine/include/sirius/LockFreeSpscQueue.h`), `kMaxTapeWriteMessageBytes` + the `TapeWriteMessage` shape (`engine/include/sirius/TapeWriter.h`), `persistence::TapeStore` (`persistence/include/sirius/TapeStore.h`), `NotificationBus` + `NotificationLevel::Error` / `Category::DiskPressure` (`engine/include/sirius/NotificationBus.h`, `core/include/sirius/INotificationSink.h`), `ChannelId` (`engine/include/sirius/Channel.h`), `Rational` (`core`).
...
**Files:**
- Create: `engine/include/sirius/IWetCaptureSink.h`
- Create: `engine/include/sirius/WetCaptureWriter.h`
...
- [ ] **Step 1: Create the interface header `engine/include/sirius/IWetCaptureSink.h`**
...
- [ ] **Step 2: Create `engine/include/sirius/WetCaptureWriter.h`**
...
./build/tests/SiriusTests "[wet-capture]"
```
...
```bash
git add engine/include/sirius/IWetCaptureSink.h engine/include/sirius/WetCaptureWriter.h \
...
Run:
```bash
grep -nE "void drain|struct Notification|level|category" engine/include/sirius/NotificationBus.h | head
```
...
**Files:**
- Modify: `engine/include/sirius/Bus.h`
- Modify: `engine/src/Bus.cpp`
- Test: `tests/WetCaptureWriterTests.cpp` (append)

- [ ] **Step 1: Add the forward declaration + setter + members to `engine/include/sirius/Bus.h`**
...
```bash
git add engine/include/sirius/Bus.h engine/src/Bus.cpp tests/WetCaptureWriterTests.cpp
