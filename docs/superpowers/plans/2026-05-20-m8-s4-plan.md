...
After the existing includes (top of the file, after the `#include "ida/IEffectChainHost.h"` line), add a forward declaration inside `namespace ida` (before `class Bus`):
...
Append an entry capturing: (a) the `Bus` wet seam is set-once plumbing not yet bound to a production capture-arm flow; (b) per-Constituent effect-chain routing (M9+) and the structure-layer `TapeId → wet-tape hash` mapping (M11 IAF) remain deferred; (c) a future refactor could extract a shared partial-writer core from `TapeWriter` + `WetCaptureWriter`. Use the existing `todo.md` entry format.
