...
**New**
- `core/include/sirius/ConstituentState.h` — the `ConstituentState` enum (header-only, no deps).
- `engine/include/sirius/ConstituentValidator.h` — `TapeResolver`, `alwaysResolves`, `ConstituentValidation`, `validate`, `postConstituentStateNotifications`, `std::hash<ConstituentId>`.
- `engine/src/ConstituentValidator.cpp` — implementations.
- `tests/ConstituentStateTests.cpp` — all `[constituent-state]` cases.

**Modified**
- `engine/include/sirius/RenderPipeline.h` — 3-arg constructor + validation member.
...
**Files:**
- Create: `core/include/sirius/ConstituentState.h`
- Create: `engine/include/sirius/ConstituentValidator.h`
...
`core/include/sirius/ConstituentState.h`:
...
`engine/include/sirius/ConstituentValidator.h`:
...
```bash
git add core/include/sirius/ConstituentState.h \
        engine/include/sirius/ConstituentValidator.h \
...
**Files:**
- Modify: `engine/include/sirius/RenderPipeline.h:50-58`
- Modify: `engine/src/RenderPipeline.cpp:35-114`
- Modify: `tests/ConstituentStateTests.cpp`

- [ ] **Step 1: Write the failing render-as-silence test**

Append to `tests/ConstituentStateTests.cpp` (add `#include "sirius/RenderPipeline.h"` and `#include "sirius/TempoMap.h"` to the includes, and `using sirius::RenderPipeline;`):
...
In `engine/include/sirius/RenderPipeline.h`, add the include and change the public/private sections:
...
```bash
git add engine/include/sirius/RenderPipeline.h engine/src/RenderPipeline.cpp \
...
Append to `tests/ConstituentStateTests.cpp` (add `#include "sirius/INotificationSink.h"`, `#include <string>`, `#include <vector>`):
...
In `app/MainComponent.cpp`, add the include near the other engine includes at the top of the file (find the existing `#include "sirius/SessionSnapshot.h"` line and add beneath it):
