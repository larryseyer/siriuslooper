...
2. Confirm the bundle has both helpers + the XPC service:
   ...
   ```bash
   find "build/app/IDA_artefacts/Release/IDA.app/Contents" \
   ...
   (The .xpc bundle is named `sirius_gui_bridge.xpc` per its `OUTPUT_NAME` in `xpc_service/CMakeLists.txt`. launchd dispatches the XPC service by `CFBundleIdentifier` = `com.larryseyer.ida.gui-bridge`, NOT by the on-disk directory name.)
