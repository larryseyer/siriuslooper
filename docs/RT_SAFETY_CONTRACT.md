...
| `sirius::OutOfProcessEffectChainHost::pumpSlot` | yes | yes | yes (O(1) unordered_map find + 1 SPSC push + 1 SPSC pop + O(numSamples × kPumpChannels) interleave/de-interleave; total payload bounded at `PluginIpcMessage::kMaxPayloadBytes = 8192` bytes) | M7 Session 3 — the audio-thread surface that promotes the engine-side producer of the M7 S2c shared-memory IPC onto the audio thread, per §5 above and the M7 S3 design decisions. `noexcept` per the `IEffectChainHost::pumpSlot` interface contract. Body shape: (1) defensive argument guards (null pointers / non-positive counts → return false); (2) `instances_.find((busId, slotIndex))` — `std::unordered_map` find is O(1) average, allocation-free, and noexcept on libc++/libstdc++ for trivially-copyable keys, AND the map is NOT mutated on the audio thread (set-once via `configureBus` on the message thread before audio starts, same M5/M6 collaborator contract); (3) interleave `inChannels` (stereo float planes) into `wireScratch_` (pre-allocated in the constructor to `PluginIpcMessage::kMaxPayloadBytes` bytes, `mutable` for the same reason `Bus::mixBuffer_` is); (4) one `OutOfProcessPluginInstance::tryWriteBytes` — wait-free SPSC push, returns false on a full ring (caller's pipelined miss is acceptable); (5) one `tryReadBytes` — wait-free SPSC pop, returns false on an empty ring (the first call after configureBus always misses; subsequent calls return the previous buffer's response = pipelined 1-buffer delay); (6) de-interleave the response payload back into `outChannels`. **Pipelined 1-buffer delay model**: on a miss, `outChannels` is left untouched (the caller — `Bus::process` — ensures `outChannels` holds the dry signal before invoking, so a miss = dry passthrough). **Single message per buffer**: at the 1024-frame envelope, `4 + 1024 × kPumpChannels × float = 8196` bytes — one byte over `kMaxPayloadBytes`. Realistic engine block sizes (64..512) fit with headroom. `JUCE-free interface` (`IEffectChainHost` lives in `core/`, no `juce::` types in the audio-thread surface). M7 Session 3 — verified by inspection plus `[output-mixer][plugin-host]` integration test asserting pipelined round-trip against the synthetic identity CLAP across 8 buffers (1040 sample assertions). Measured baseline: inherits the 68 µs median / 134 µs p99 round-trip from the S2c `[plugin-ipc][.rt-smoke]` audit (pumpSlot adds only the in-place interleave/de-interleave; the IPC transport cost dominates). |
