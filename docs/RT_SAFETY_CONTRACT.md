...
| `sirius::FlacTapeSink::deliverTapeBlock` | yes | yes | yes (O(numSamples) interleave copy + 1 SPSC push + 1 relaxed atomic increment) | Tape subsystem slice 3 (2026-05-21) — the audio-thread `ITapeSink` implementation that feeds the FLAC worker. `noexcept` per the `ITapeSink` contract and enforced by signature. Body: (1) clamp `numSamples` to `kFlacSinkMaxFramesPerMessage` and early-return on non-positive count or null pointers; (2) construct a `Message` on the stack — a trivially-copyable POD whose `samples` array is NOT zero-initialised (the `{}` was removed: `deliverTapeBlock` fills exactly `[0, numFrames*2)` before push, so no uninitialized bytes reach the worker); (3) interleave `left[]` / `right[]` into `msg.samples` via a tight loop bounded by the clamped frame count; (4) one wait-free `LockFreeSpscQueue::push` — returns `false` on full (overflow path bumps `droppedBlocks_` via relaxed `fetch_add`; no other action on the audio thread). **No allocation, no lock, no I/O, no `throw`.** FLAC encoding and file I/O are entirely on the worker thread; the audio thread never touches a `juce::AudioFormatWriter`. No `wakeCv_` notify on the audio thread (would require a mutex lock on the hot path) — the worker's `wait_for` timeout bounds maximum delivery latency to 20 ms. Tape subsystem slice 3 — verified by inspection plus `[flac-tape-sink]` test suite (happy-path delivery, queue-overflow drop counting). |
